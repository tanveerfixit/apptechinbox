    ...
    <main class="container-fluid px-2 px-sm-3 py-3 py-md-4 flex-grow-1" style="max-width: 600px; margin: 0 auto;" 
          x-data="{
              ...
              init() {
                  ...
                  this.$nextTick(() => {
                      const intakeUrl = window.location.origin + '/intake.php?session_id=' + this.sessionId + '&t=' + this.timestamp;
                      new QRious({
        ...
        <div class="card shadow-sm border-1 p-3 mb-3 bg-white text-center" style="border-radius: 6px; border-color: var(--card-border) !important;">
            ...
            <div class="small text-muted d-flex align-items-center justify-content-center gap-2" style="font-size: 11px;">
                ...
                <button type="button" @click="navigator.clipboard.writeText(window.location.origin + '/intake.php?session_id=' + sessionId + '&t=' + timestamp); alert('Intake link copied to clipboard!')" class="btn p-0 border-0 d-inline-flex align-items-center" title="Copy Intake Link" style="color: var(--brand-teal); transition: opacity 0.15s ease;" onmouseover="this.style.opacity='0.7'" onmouseout="this.style.opacity='1'">
